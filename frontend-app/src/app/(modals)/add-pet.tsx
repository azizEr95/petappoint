...
import { addRacesToAnimal } from '@src/api/animalRaces'
...
export default function AddPet() {
  ...
  const { data: animalRaces } = useAnimalRaces(animalTypeId)
  ...
  function handleSave() {
    ...
    create(
      ...
      {
        onSuccess: async (newAnimal) => {
          if (animalRaceId) {
            try {
              await addRacesToAnimal(newAnimal.id, [animalRaceId])
            } catch {
              // Rasse konnte nicht gespeichert werden – trotzdem weiter
            }
          }
          router.back()
        },
  ...
  const isSubmitDisabled = isPending || !animalTypeId || !name
  ...
  return (
    <Box className='flex-1 bg-background-100'>
          ...
          <VStack className='gap-4'>
            ...
            <Card className='bg-background-0 rounded-xl shadow-sm p-4'>
              ...
              <FormControlLabel className='mb-3'>
                <FormControlLabelText size='sm' className='font-semibold text-typography-500 uppercase'>{t('add_pet.animal_type')}</FormControlLabelText>
              ...
              >
                <Text className={selectedType ? 'text-typography-900' : 'text-typography-400'}>
                  {selectedType?.name ?? t('add_pet.type_placeholder')}
                </Text>
            ...
            {/* Rasse (optional, nur wenn Tierart gewählt) */}
            {animalTypeId !== null && (
              <Card className='bg-background-0 rounded-xl shadow-sm p-4'>
                <Text size='sm' className='font-semibold text-typography-500 uppercase mb-3'>
      ...
      {/* Tierart Actionsheet */}
      <Actionsheet isOpen={typeSheetOpen} onClose={() => setTypeSheetOpen(false)} useRNModal>
        <ActionsheetBackdrop />
        <ActionsheetContent>
          <ActionsheetDragIndicatorWrapper>
            <ActionsheetDragIndicator />
          ...
          <ActionsheetFlatList
            ...
            renderItem={({ item }) => {
              const type = item as { id: number; name: string }
              return (
                <ActionsheetItem
                  onPress={() => {
                    setAnimalTypeId(type.id)
                    setAnimalRaceId(null)
                    setTypeSheetOpen(false)
                  }}
                >
                  <ActionsheetItemText className={animalTypeId === type.id ? 'text-primary-500 font-semibold' : ''}>
                    {type.name}
                  </ActionsheetItemText>
                </ActionsheetItem>
              )
