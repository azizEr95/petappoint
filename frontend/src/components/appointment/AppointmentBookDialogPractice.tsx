import { Alert, Button, Form, FormGroup, ListGroup, Modal, Spinner } from 'react-bootstrap';
...
import type { AnimalTypeType, AppointmentsType, PersonsType } from 'vetilib-shared/schemas/ZodSchemas';
...
export function AppointmentBookDialogPractice({
    ...
    const { data: treatableAnimalTypesData, isLoading: isLoadingAnimalTypes } = useQuery({
        queryKey: ['treatableAnimalTypes', vetId],
        queryFn: () => getAnimaltypesFromVeterinary(vetId.toString()),
        retry: false,
    });

    const { data: allAnimalTypesData, isLoading: isLoadingAllAnimalTypes } = useQuery<Array<AnimalTypeType>>({
    ...
    const filteredAnimals = useMemo(() => {
        if (!selectedPersonId || !personAnimalsData || treatableAnimalTypeIds.length === 0) return [];
        return personAnimalsData
            .filter(animal => treatableAnimalTypeIds.includes(animal.animalTypeId));
    ...
    const { mutate: mutateBookAppointment, isPending: isBookingPending } = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['availableAppointmentsPractice'] });
            hideDialogBookAppointment();
        },
        onError: (error: Error) => {
    ...
    const isDataLoading = isLoadingCustomers || isLoadingAnimalTypes || isLoadingAllAnimalTypes;
    const isFormInvalid = !selectedServiceId || !selectedPersonId || !selectedAnimalId;
    ...
    const handleServiceChange = (selectedOption: SingleValue<{value: number, label: string}>) => {
        setSelectedServiceId(selectedOption?.value.toString() || '');
    }

    const handlePersonChange = (selectedOption: SingleValue<{value: number, label: string}>) => {
        setSelectedPersonId(selectedOption?.value || null);
        setSelectedAnimalId(null);
    }

    const handleAnimalChange = (selectedOption: SingleValue<{value: number, label: string}>) => {
    ...
    const handlePersonCreated = (personId: number) => {
    ...
    const handleAnimalCreated = () => {
        queryClient.invalidateQueries({ queryKey: ['customers', practiceId] });
        queryClient.invalidateQueries({ queryKey: ['personAnimals', selectedPersonId] });
    ...
    const selectedAnimal = useMemo(() => {
        ...
        return personAnimalsData.find(animal => animal.id === selectedAnimalId) || null;
        ...
        >
            ...
            <Modal.Body>
                ...
                {!customersData?.length && !isLoadingCustomers && (
                    <Alert variant="info">
                        Keine Kunden gefunden. Diese Praxis hat noch keine Kunden mit gebuchten Terminen.
                    </Alert>
                )}
                    ...
                    <ListGroup.Item>
                        <FormGroup>
                            ...
                            <Select
                                ...
                                value={personOptions.find(opt => opt.value === selectedPersonId) || null}
                        ...
                        <FormGroup>
                            ...
                            {selectedPersonId && filteredAnimals.length === 0 ? (
                                ...
                                <Select
                                    ...
                                    value={animalOptions.find(opt => opt.value === selectedAnimalId) || null}
                                    onChange={handleAnimalChange}
                                    placeholder="Tier auswählen..."
                                    isClearable
                                    isDisabled={!selectedPersonId || isDataLoading || isBookingPending}
                                    styles={customSelectStyles}
                                />
                            )}
                        </FormGroup>
                        ...
                        <FormGroup>
                            ...
                            {selectedAnimalId && !canTreatSelectedAnimal ? (
                                ...
                                <Select
                                    ...
                                    value={serviceOptions.find(opt => opt.value.toString() === selectedServiceId) || null}
                                    onChange={handleServiceChange}
                                    placeholder="Behandlung auswählen..."
                                    isClearable
                                    isDisabled={!selectedAnimalId || isDataLoading || isBookingPending}
                                    styles={customSelectStyles}
                                />
                            )}
                        </FormGroup>
            ...
            <Modal.Footer>
                <Button
                    variant="secondary"
                    onClick={hideDialogBookAppointment}
