...
export const getVeterinariansByPracticeId = async (
  practiceId: string,
): Promise<Array<VeterinariansType>> => {
...
export const createVeterinarian = async (
  veterinarian: VeterinariansCreateType,
): Promise<VeterinariansType> => {
  const res = await fetch(import.meta.env.VITE_API_URL + '/veterinarians', {
    method: 'POST',
...
export const checkPersonByEmail = async (
  email: string,
): Promise<CheckPersonType> => {
  const body = { email: email }
  const res = await fetch(import.meta.env.VITE_API_URL + '/persons/email', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export const getVeterinarianById = async (
  ...
  const res = await fetch(
    import.meta.env.VITE_API_URL + '/veterinarians/' + id,
    { credentials: 'include' },
  )
  if (!res.ok) {
...
export const updateVeterinarian = async (
  id: number,
  data: Partial<VeterinariansType>,
): Promise<VeterinariansType> => {
  const res = await fetch(
    import.meta.env.VITE_API_URL + '/veterinarians/' + id,
    {
  ...
  if (!res.ok) {
    throw new Error('Failed to update veterinarian')
  }

  const responseData = await res.json()
  const parsed = VeterinariansSchema.safeParse(responseData)
  if (!parsed.success) {
    console.log(parsed.error)
    throw new Error('Invalid veterinarian response')
  }
...
const parseVeterinariansArray = (
  unsafeVeterinarians: Array<VeterinariansType>,
